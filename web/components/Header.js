...
export default function Header({ leads, config, onConfigSave }) {
      ...
      <header className="bg-white border-b shadow-sm sticky top-0 z-40">
        <div className="px-6 py-3 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            {config && (
              <span className="text-gray-400 text-sm ml-1">{config.city} · {config.state}</span>
            )}
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-gray-50 border-gray-200 hover:bg-gray-100 transition-colors"
              title="Configurações da busca"
            >
              ⚙️ <span className="text-gray-600">Config</span>
            </button>
          </div>
        </div>
      </header>
